	...
	*/
	public function checkLogin()
	{
		...
		if (isset($_POST['retrievePassword'])) 
		{
			...
			// search for info
			$rs = $OBJ->db->fetchRecord("SELECT ID, email, userid, user_name, user_surname 
				FROM ".PX."users 
		...
		if (isset($_POST['submitLogin'])) 
		{
			...
			$clean['userid'] 	= getPOST('uid', null, 'connect', 12);
			$clean['password'] 	= md5(getPOST('pwd', null, 'connect', 12));
			$clean['user_active'] = 1;
